...
#define PROBE_VERSION "0.1.0-dev.12"

typedef enum {
    PROBE_MODE_NATIVE_CONTROL = 0,
    PROBE_MODE_NATIVE_ICV_READ = 1,
    PROBE_MODE_FORCE_ICV_FLAG = 2
} probe_mode_t;

typedef struct {
    char romver[32];
    char model[32];
    char model_raw_hex[33];
    int model_result;
...
typedef struct {
    ...
    int icvps2_read_attempted;
    int icvps2_read_unconditional;
